/*
  validaccion.js
  ----------------
  Script único para validar formularios de Login y Registro.

  Reglas importantes:
  - NO usamos RegExp ni Constraint Validation API (requisito de la práctica).
  - Validaciones manuales por campo: username, password, email, sexo, ciudad/pais,
    foto y fecha de nacimiento (edad mínima 18 años).
  - El script inserta mensajes de error junto al input (span.error-msg).
  - Para depuración usa console.warn en los puntos donde se marca un error.

  Mantén este fichero ligado a tus HTML mediante <script src="js/validaccion.js"></script>

*/

(function(){
  /****************************************************** */
  // UTILIDADES COMPARTIDAS (usadas por login y registro)
  /****************************************************** */
  function $(id){ return document.getElementById(id); }

  // --- creación / gestión de mensajes de error ---
  // Crea o reutiliza un <span class="error-msg"> inmediatamente después del
  // input. Todos los mensajes de validación se muestran ahí para facilitar
  // el estilo y la localización por CSS. No modifica el contenido del input.
  ...
  // Añade un mensaje de error visual al input y le aplica la clase
  // `.input-error`. Se hace console.warn para desarrollo; no detiene el flujo.
  // La función que llama a marcarError es la responsable de cancelar el submit
  // (mediante event.preventDefault()) si corresponde.
  ...
  // DEBUG: registro en consola para depuración. Quitar o desactivar en producción.
  try{ console.warn('[VALIDATION] marcarError', input && input.id, mensaje); }catch(e){}
  }
  ...
  // - Ciudad/País: no vacíos
  // - Foto: debe seleccionarse un archivo
  ...
  // Reglas implementadas (sin regex):
  ...
  if(email){
      ...
      if(!emailValid){
        // DEBUG: indicar por consola que el email fue considerado inválido.
        // Permanece para facilitar pruebas; eliminar en despliegue si no se desea logging.
  ...
  // Inicialización: asocia handlers de submit y controla eventos de input
  // para limpieza en tiempo real. También inicializa los selects de fecha
  // (month/day/year) y sincroniza el campo oculto `birthdate`.
  function init(){
